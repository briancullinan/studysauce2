    ...
    public function activityAction(Request $request)
    {
        ...
        $visits = array_map(function (Visit $v) {
                return [
                    ...
                    'content' => '<a href="#visit-id-' . $v->getId() . '"><strong>Path:</strong><span> ' . $v->getPath() . '</span><br /><strong>User:</strong><span> ' . (!empty($v->getUser()) ? $v->getUser()->getEmail() : 'Guest') . '</span><br /><strong>Session Id:</strong><span> ' . (!empty($v->getSession()) ? $v->getSession() : 'New session') . '</span></a>',
